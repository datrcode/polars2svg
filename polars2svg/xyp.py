...
class XYp(P2SBackgroundMixin, ExportMixin):
    ...
    def __renderLines_complex__(self, df, _enums_, _color_, _dash_array_, _width_, _randid_):
        ...
        if _per_endpoint_:
            ...
            # Make sure the line is safe to use as an svg id. Allowlist: replace
            # every char that is not [A-Za-z0-9] with '-'. This is an allowlist,
            # not a blocklist, so XML-special chars (< > & ") in the untrusted
            # __line__ (line-by) value cannot break out of the id="..." attribute
            # or the url(#...) reference. Uniqueness is guaranteed independently by
            # the __uniq__ row-index suffix on the id, so any collisions the
            # replacement introduces here are harmless.
            _make_safe_ = pl.col('__line__').str.replace_all('[^A-Za-z0-9]', '-').alias('__line_safe__')
