...
class Testxyp_distributions(unittest.TestCase):
    ...
    def test_basics(self, samples=1, df_size=200):
        ...
        self.p2s.xyp(**_params_, x_distributions='a')
        self.p2s.xyp(**_params_, x_distributions=('a','b'))
        self.p2s.xyp(**_params_, x_distributions=['a','b'])
        self.p2s.xyp(**_params_, x_distributions=['a','b', self.p2s.SETp])

    def test_rowCounts(self, sample=1, df_size=200):
        df       = randomDataFrame(df_size)
        _params_ = {'df':df, 'x':'a', 'y':'b'}
        self.p2s.xyp(**_params_, x_distributions=self.p2s.ROW_COUNTp)
        self.p2s.xyp(**_params_, x_distributions=self.p2s.ROW_COUNTp, y_distributions=self.p2s.ROW_COUNTp)
        self.p2s.xyp(**_params_,                                      y_distributions=self.p2s.ROW_COUNTp)
    ...
    def test_constantDistributionField(self):
        ...
        self.p2s.xyp(df=df, x='x', y='y', x_distributions='v')
        # y_distributions on a constant column
        self.p2s.xyp(df=df, x='x', y='y', y_distributions='v')
        # both axes at once
        self.p2s.xyp(df=df, x='x', y='y', x_distributions='v', y_distributions='v')
        ...
        for _dot_size_ in [None, 1, 4, 10, 0.75]:
            for _x_dist_ in [None, 'a', ('a', self.p2s.DISTRIBUTION_INSIDEp), ('a', self.p2s.DISTRIBUTION_OUTSIDEp)]:
                for _y_dist_ in [None, 'b', ('b', self.p2s.DISTRIBUTION_INSIDEp), ('b', self.p2s.DISTRIBUTION_OUTSIDEp)]:
                    self.p2s.xyp(x_distributions=_x_dist_, y_distributions=_y_dist_, dot_size=_dot_size_, **_params_)
